...
let keys = [8,9,13,16,17,18,27,32,33,34,35,36,37,38,39,40,46,91,93,112,113,114,115,116,117,118,119,120,121,122,123];
...
function burst(intensity) {
    ...
    let force = .7;
    let lifeMin = 0;
    let progress = Math.min(field.width, caret.offsetWidth) / field.width;
    let offset = field.left + (field.width * progress);
    let rangeMin = Math.max(field.left, offset - 30);
    let rangeMax = Math.min(field.right, offset + 10);
